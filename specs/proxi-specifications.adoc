...
2. ProXI should be able to provide mass spectrometry evidences deposited in public repositories for peptides and proteins. If a researcher (e.g. Biologist) is interested to know if a `peptide` or a `protein` has been observed before in a previous deposited dataset it should be possible to retrieve mass spectrometry evidence (MS/MS spectrum) that support that evidence. It should be possible to retrieve additional information such as search engine scores, and statistical assessments values.
...
4. ProXI API should be possible to retrieve information about independent experiment as long as aggregation analysis of multiple experiments. For example, ProXI API can be implemented by PeptideAtlas that report `peptide` and `protein` for groups or collections of datasets; but also can be implemented by PRIDE database that provide `peptide` and `protein` evidences at dataset (study) level.
...
length of URIs as well. No reason to keep them. If the API is retrieving multiple file formats (e.g. JSON and XML). This can be specified by a parameter or in the request header.
...
- A Compact object is a version of the object that only contains the required fields of the Object type. For example, for a peptide spectrum match would be the `peptideSequence` or for an spectrum the `unified spectrum identifier`.

- A Full object is a version of the object that contains all the attributes/fields of the Object type.

6. We RECOMMENDED the use of **Compact** version of an object as default behavior for collections end-points, for example:
...
The filtering will be defined as the <propertyName>=<propertyValue> where the propertyName is the name of the attribute that would be use for filtering and the propertyValue is the possible value the user would like to filter the collection.

If multiple values would like to be filter use to filter the collection, the combination MUST be specified like `value1,value2`
